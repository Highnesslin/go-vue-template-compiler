...
func Parse(template string) ASTNode {
	 ...
	 * 向栈内追加子节点
	 */
	pushChildrenToCurrentParent := func(arg interface{}) {
		stackLen := len(stack)
		if stackLen > 0 {
		...
		for {
			...
			if len(match) > 0 {
				...
				if len(match[0]) == 2 {
					...
					// token = node.tagName
					// 收集 ASTNode 的信息
					pushChildrenToCurrentParent(node)
				} else {
					pushStack(node)
				}
				break
	...
	parseText := func() {
		...
		if len(text) > 0 {
			...
			pushChildrenToCurrentParent(TextNode{PLAIN_TEXT, text})
		}
	}
	...
	parseInterpolation := func() {
		...
		if len(match) > 0 {
			...
			pushChildrenToCurrentParent(TextNode{INTERPOLATION, match[1]})
		}
	}
		...
		if template[0] == '<' {
			...
			if len(match) > 0 {
				...
				if err != nil {
					...
					pushChildrenToCurrentParent(last)
				}
			}
